...
- Gold table = 10,088 facilities × ~10 capabilities ≈ **100k rows, tens of MB.**
...
- https://www.who.int/publications/i/item/WHO-HIS-HSI-2014.5-Rev.1
...
- https://nhsrcindia.org/sites/default/files/CHC%20IPHS%202022%20Guidelines%20pdf.pdf
...
- CRH (SIGMOD 2014) — https://cse.buffalo.edu/~jing/doc/sigmod14_crh.pdf
...
Precomputing an LLM justification per (facility, capability) is 10,088 × 10 ≈ **100k LLM calls** —
...
Two numbers were in circulation: ~100 capabilities (cell key space) and 10 (tracer scoring).
...
| **Tracer-scored capabilities** | **10** | Have SARA domains and IPHS tracer items defined. These get readiness scores |
| **Semantic tail** | unbounded | Everything the vocabulary misses → vector search over `distinct_bullets` |

**Scope discipline: ten tracer-scored capabilities. Defined once. No additions after the first
pass.** The vocabulary can grow freely; the scored set cannot.
...
**Method.** SARA composite. Per domain, the fraction of tracer items evidenced. Overall
readiness = mean of domain scores. **Report both the mean and the conditional score** (whether
every minimum-standard item is met). Layer IPHS-derived consistency rules as contradiction flags
(advanced surgery with no anaesthesia/OT evidence; ICU with no ventilator evidence anywhere).
...
- **Ten tracer-scored capabilities.** No additions after the first pass.
- **Free Edition only.**
